...
export var setup_top_apps = function (json_str) {
  ...
  var make_chart_min_max = function (appname, xaxis, yaxis, x_scale, y_scale, chart_id, obj, xmin, xmax, ymin, ymax) {
    ...
    var element = React.createElement(Chart, {container: chart_id, options: options});

    if (typeof window !== 'undefined') {
      ReactDOM.render(element, document.getElementById(chart_id));
    }
  }
  ...
  var make_chart = function (appname, xaxis, yaxis, x_scale, y_scale, chart_id, obj) {
    ...
    console.log(Chart);
    console.log(options);
    var element = React.createElement(Chart, {container: cont, options: options});
